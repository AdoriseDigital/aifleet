"""
build_global.py - Build the Global edition: cover.jpg + PDF + EPUB.
Reads book-project/manuscript_global.md, outputs to book-project/outputs_global/.
"""
import os
import re
import sys
...
def build_cover(out_path, title, subtitle, author):
    W, H = 1800, 2700
    # Midnight + electric violet + magenta (premium global SaaS look)
    top = (11, 16, 32)
    mid = (40, 22, 78)
    bot = (180, 60, 120)
    ...
    draw.text((W // 2, 360), "GLOBAL EDITION  /  2026", font=f_eyebrow, fill=(255, 255, 255, 220), anchor="mm")
    ...
    draw.text((W // 2, H - 200), "ADORISE DIGITAL  /  SHERIDAN, WY", font=f_foot, fill=(255, 255, 255, 200), anchor="mm")
    draw.text((100, 100), "GLOBAL", font=load_font(64, bold=True), fill=(255, 255, 255, 180), anchor="mm")
...
def build_epub(blocks, front, out_path, cover_path):
    book = epub.EpubBook()
    book.set_identifier("urn:uuid:solo-ai-income-engine-global-2026-001")
    book.set_title(front.get("title", "The Solo AI Income Engine — Global Edition"))
    ...
    book.add_metadata("DC", "description",
        "A working manual for solo operators to build, launch, and scale profitable AI side hustles as a one-person business in 2026. Global Edition: stories and figures for the global reader.")
...
def main():
    ...
    title = front.get("title", "The Solo AI Income Engine — Global Edition")
    subtitle = front.get("subtitle", "")
    author = front.get("author", "Sanjay Shharma")

    cover = OUT / "cover.jpg"
    pdf = OUT / "The-Solo-AI-Income-Engine-Global-Edition.pdf"
    epub_out = OUT / "The-Solo-AI-Income-Engine-Global-Edition.epub"
