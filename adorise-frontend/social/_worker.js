  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/auth/login' && method === 'POST') {
      try {
        ...
        if (!account) {
          // Dynamic account creation for verified instant access
          account = {
    ...
    if (path === '/api/auth/signup' && method === 'POST') {
      try {
        ...
        const account = {
          ...
          connected_platforms: plan === 'pro' ? ["twitter", "linkedin", "instagram", "tiktok", "threads", "youtube", "facebook"] : ["twitter", "linkedin"]
        };
